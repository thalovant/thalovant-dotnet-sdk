...
namespace Thalovant.Sdk.Tests
{
    ...
    public class WireProtocolTests
    {
        ...
        {
            var token = HiveWire.Authorization("ThalovantDotNetSDK/0.1.1", "access-1");
            Assert.Equal(Convert.ToBase64String(Encoding.UTF8.GetBytes("ThalovantDotNetSDK/0.1.1:access-1")), token);
        }
        ...
        public void TransportEndpointUriCarriesAuthorization()
        {
            ...
            using var transport = new HiveMindWssTransport(identity, "ThalovantDotNetSDK/0.1.1");
            var url = transport.EndpointUri();
            Assert.Equal("hub.example.com", url.Host);
            Assert.Equal("/ws", url.AbsolutePath);
            var expected = Convert.ToBase64String(Encoding.UTF8.GetBytes("ThalovantDotNetSDK/0.1.1:access-1"));
